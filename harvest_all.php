...
//pass 0 for current date and so on
$first = $argv[1];
$second = $argv[2];
$startdate = date('Y-m-d',strtotime("+$first day")); 
$enddate = date('Y-m-d',strtotime("+$second day"));
...
$processChilds = 6 ;  //set n for n childs
	...
	while($row = mysqli_fetch_array($result) )
	{
		...
		for($a = 0 ; $a < 2 ; $a++ )
		{
			...
			for($am = 1 ; $am < $processAdults ; $am++ )  
			{
				...
				{
					$childflag = true ;
					echo "==> <strong>Job: Date ".$startdate." Resort ".$resortcount." Adults ".$am." Childs ".$cm." Accessible ".$a."</strong>"."  at ".date(DATE_COOKIE,time());  
...
{
	echo "could not fetch resort names";
}
echo "<strong>JOB OVER for ".$startdate." to ".$enddate."</strong> "."  at ".date(DATE_COOKIE,time())."<br>" ;
?>
